...
public class MatchStatEditor {
    ...
    public static void setDuelStats(Profile winner, Profile loser, Ladder ladder, boolean ranked) {
        if (ranked) {
            winner.getLadderRankedWins().put(ladder, winner.getLadderRankedWins().getOrDefault(ladder, 0) + 1);
            winner.setRankedWins(winner.getRankedWins() + 1);

            loser.getLadderRankedLosses().put(ladder, loser.getLadderRankedLosses().getOrDefault(ladder, 0) + 1);
            loser.setRankedLosses(loser.getRankedLosses() + 1);
        } else {
            winner.getLadderUnRankedWins().put(ladder, winner.getLadderUnRankedWins().getOrDefault(ladder, 0) + 1);
            winner.setUnrankedWins(winner.getUnrankedWins() + 1);

            loser.getLadderUnRankedLosses().put(ladder, loser.getLadderUnRankedLosses().getOrDefault(ladder, 0) + 1);
